...
v2 拉霸結果頁已有 GA `postMessage` 埋點，但營運端需要另一支 usage API，記錄使用者在結果頁的後續動作（推薦出現、釘選、刷新、點商品跳轉、重開、關閉），並帶上當下畫面三件商品與釘選狀態。對應 lambda：`mkt_extensions_usage_record` → `{Brand}_Extensions_Record`。
...
觸發點掛在既有拉霸流程：`show_results`、`toggleCapsulePin`、`#recommend-btn`、`#container-recom .reel-link`、`#startover`、結果頁 `closeModal`／`pagehide`／`beforeunload`。
...
| Action | `ProductInfo` | `ProductCategory` | `ActionPtr` | `RedirectPtr` |
|--------|---------------|-------------------|-------------|---------------|
...
- `RedirectPtr`：僅 `Redirect`；被點欄位在 `reelCats`／`ProductInfo` 的 0-based index（數字，非陣列），例如點第三欄送 `2`，對應商品為 `ProductInfo[2]`。無法對應時不寫入。
