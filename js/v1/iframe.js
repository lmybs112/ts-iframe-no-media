    ...
    function bind() {
      ...
      for (var fs = 0; fs < all_Route.length; fs++) {
        (function (fs) {
          ...
          // 僅非父層續選的舊路徑才自動 click 推進；父層續選已用 applyResumeQuestionUi
          if (
            !suppressPresetResume &&
            !useParentSelectionRestore &&
...
$("#recommend-btn").on(tap, async function () {
  ...
  if (firstResult.Item?.length <= 3) {
    ...
    $("#recommend-title").text(uiT("recommend.titleMore"));
    $("#recommend-desc").text(uiT("recommend.descMore"));
